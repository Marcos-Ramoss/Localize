## LOCALIZE+
   ### É um projeto que foi criado para apresentação na feira innovatec da fametro para obtenção de nota, esse aplicação Web é voltado para facilitar a localização de produtos em determinado estabelecimento  possui um **QR CODE** na pagina inicial para o cliente acessar a aplicação.
   - Possui niveis de acesso usuario e admin
   - Possui Banco de Dados e criptografia na senha
   - Admin pode excluir usuario e cadastrar novos produtos dentro da empresa que adotar a aplicação
   - Usuario pode apenas navegar na consulta para localizar o produto especifico
   - Aplicação conta com um sistema CRUD de cadastro
   - Possui 3 APIS do google maps (API PLACES, API ROUTERS,  API GEOCODIFICAÇÃO)
   - Possui validação de usuario ja cadastrado evitando duplicidade de usuario
